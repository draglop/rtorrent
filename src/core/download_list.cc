...
void
DownloadList::clear() {
  std::for_each(begin(), end(), std::bind(std::mem_fn(&DownloadList::close), this, std::placeholders::_1));
...
DownloadList::session_save() {
  unsigned int c = std::count_if(begin(), end(), std::bind(std::mem_fn(&DownloadStore::save_resume), control->core()->download_store(), std::placeholders::_1));
